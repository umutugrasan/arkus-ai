    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <GlassCard className="bg-gradient-to-br from-indigo-500/20 to-indigo-600/10">
          <p className="text-slate-400 text-xs uppercase tracking-wider">Toplam Fırsat</p>
          <p className="text-4xl font-bold text-white mt-1">{summary?.total_opportunities ?? 0}</p>
        </GlassCard>
        <GlassCard className="bg-gradient-to-br from-emerald-500/20 to-emerald-600/10">
          <p className="text-slate-400 text-xs uppercase tracking-wider">Aylık Potansiyel</p>
        ...
        <GlassCard className="bg-gradient-to-br from-violet-500/20 to-violet-600/10">
          <p className="text-slate-400 text-xs uppercase tracking-wider">En Büyük Fırsat</p>
          <p className="text-white font-semibold mt-1 text-sm">{summary?.biggest_gap_product ?? '—'}</p>
        </GlassCard>
      </div>
                ...
                <GlassCard key={i} className={`cursor-pointer transition-all ${isOpen ? 'border border-indigo-500/40' : 'hover:border-slate-600'}`}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-white font-semibold">{opp.product_name || opp.product_id}</p>
                        <p className="text-slate-400 text-xs">{opp.category}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        <p className="text-emerald-400 font-bold text-lg">{formatCurrency(opp.monthly_opportunity)}</p>
                        <p className="text-slate-400 text-xs">aylık potansiyel</p>
                      </div>
                          ...
                          <div className="space-y-3">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                              {(detail.listings || []).map((l: ArbitrageListing, li: number) => (
                                <div key={li} className="p-3 bg-slate-800/40 rounded-xl">
                                  <MarketplaceBadge marketplace={l.marketplace} />
                                  <p className="text-white font-bold text-lg mt-2">{formatCurrency(l.price)}</p>
                                  <div className="space-y-1 mt-2 text-xs text-slate-400">
                                    ...
                                    <p>Marj: <span className="text-white">{l.net_margin_pct.toFixed(1)}%</span></p>
