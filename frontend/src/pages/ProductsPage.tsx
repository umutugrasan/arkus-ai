    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <GlassCard className="p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-white font-semibold">En Çok Satanlar</h3>
          </div>
          {topSellers.length === 0 ? (
              ...
              {topSellers.map((p, i) => (
                <li key={p.id}>
                  <Link
                    ...
                    className="flex items-center justify-between gap-2 p-2.5 rounded-lg bg-slate-800/40 hover:bg-slate-800/80 transition-colors group"
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-slate-500 text-[11px]">{formatNumber(p.total_sales)} satış</p>
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      ...
                      <p className="text-slate-500 text-[10px]">net kâr</p>
                    </div>
                  </Link>
                </li>
              ))}
        ...
        <GlassCard className="p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-white font-semibold">Stoku Kritik</h3>
            <span className="ml-auto text-xs text-slate-500">{lowStock.length} uyarı</span>
          </div>
          {lowStock.length === 0 ? (
      ...
      <GlassCard className="p-5">
        <div className="flex flex-wrap items-center gap-3 mb-4">
          <h3 className="text-white font-semibold flex-1">Tüm Ürünler</h3>
          <Input
          ...
          <div className="relative">
            ...
            <select
              ...
              className="bg-slate-800/60 border border-slate-700/60 rounded-xl pl-8 pr-3 py-2.5 text-sm text-white outline-none"
            >
                ...
                {filtered.map((p) => (
                  ...
                  >
                    <td className="py-2.5 pl-2">
                      <Link
                        to={`/products/${encodeURIComponent(p.id)}`}
                        className="text-white font-medium hover:text-indigo-300 max-w-[260px] truncate block"
                      >
                        {p.name}
                      </Link>
                      <p className="text-slate-500 text-[10px]">{p.id}</p>
                    </td>
                    <td className="py-2.5 text-slate-400 text-xs">{p.category}</td>
                    <td className="py-2.5 text-center text-amber-400 text-xs">
                      ⭐ {p.rating?.toFixed(1) || '—'}
                      <p className="text-slate-500 text-[10px]">{formatNumber(p.review_count)}</p>
                    </td>
                    ...
                    <td className="py-2.5 text-right text-slate-300">{formatCurrency(p.avg_profit_per_item)}</td>
                    <td className="py-2.5 text-right text-slate-300">{formatNumber(p.total_stock)}</td>
                    ...
                    <td className="py-2.5 pr-2 text-right">
                      <Link
                        to={`/products/${encodeURIComponent(p.id)}`}
                        className="inline-flex p-1 rounded text-slate-400 hover:text-indigo-400 hover:bg-indigo-500/10"
                      >
