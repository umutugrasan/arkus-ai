...
export function ThemeToggle({
  ...
  // Mount'ta: <html>.dark sınıfını veya localStorage'i okuyup state'i hizala.
  // (index.html'deki pre-paint script bunu zaten <html>'e koymuş olur — biz yalnız sync ediyoruz.)
  useEffect(() => {
    if (typeof document === "undefined") return;
    const stored = (typeof localStorage !== "undefined" && localStorage.getItem("arkus_theme")) as Theme | null;
    const root = document.documentElement;
    const hasDarkClass = root.classList.contains("dark");
    const resolved: Theme = stored ?? (hasDarkClass ? "dark" : "light");

    if (resolved === "dark" && !hasDarkClass) root.classList.add("dark");
    if (resolved === "light" && hasDarkClass) root.classList.remove("dark");
    if (theme !== resolved) setTheme(resolved);
  ...
  const toggle = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      if (typeof localStorage !== "undefined") {
        try { localStorage.setItem("arkus_theme", next); } catch { /* quota / private mode */ }
      }
