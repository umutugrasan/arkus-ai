...
def _build_by_marketplace(all_metrics: dict) -> list[dict]:
    result = []
...
def _build_by_product(all_metrics: dict) -> list[dict]:
    products: dict = {}
    for _mp, metrics in all_metrics.items():
...
def _build_expenses(overall: dict) -> dict:
    total_expense = (
        overall["total_cost"]
...
def _build_cash_flow(overall: dict, history: list) -> dict:
...
@router.get("/full")
def financials_full(user=Depends(get_current_user), db=Depends(get_db)):
    """
    Tek pass'te tum finansal verileri doner: overview, by_marketplace, by_product,
    expenses, cash_flow. FinancialsPage'in 5 paralel istegi yerine 1 istek.
    """
    ...
    return {
        "overview": {"overall": overall, "monthly_history": monthly_history},
        "by_marketplace": {"marketplaces": _build_by_marketplace(all_metrics)},
        "by_product": {"products": _build_by_product(all_metrics)},
        "expenses": _build_expenses(overall),
        "cash_flow": _build_cash_flow(overall, history),
    }
